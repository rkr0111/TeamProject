...
import com.TeamPro.action.Review_ContentsInsertAction;
...
public class Light_controller extends javax.servlet.http.HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		}else if(command.equals("/buyHistoryInsert.bo")) { // 1005 dhdbswl 수정
			action = new Buy_ProInsertAction();
			try{
				forward=action.execute(request, response);
			}catch(Exception e){
				System.out.println(e);
		...
		}else if(command.equals("/reviewContentsInsert.bo")) { // 1008 dhdbswl 수정
			action = new Review_ContentsInsertAction();
			try{
				forward=action.execute(request, response);
			}catch(Exception e){
				System.out.println(e);
